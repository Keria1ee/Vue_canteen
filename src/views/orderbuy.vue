...
import {buyPreorder, getPreorderList} from '@/api/preorder.js'
...
const buyorder = async (row) => {
  ...
  let result = await buyPreorder(payload.value);
  if (result.success === 1) {
    ElMessage.success('你已经成功购买了' + row['dishname'] + ',数量为:' + row.quantity + ',总价为' + row.quantity * row.price + '元');
    getCategorys();
  }else{
    ElMessage.error(result.message?result.message:'购买失败');
    getCategorys();
  }
}
...
const getCategorys = async () => {
  ...
  if (result.success === 1) {
    categorys.value = result.data.map(category => ({
      ...category,
      quantity: '' ,
      description: ''// 添加 quantity 属性，初始值为 0
    }));
  } else {
